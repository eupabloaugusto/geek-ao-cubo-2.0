...
$aria_label  = isset( $args['aria_label'] )
	...
	: esc_attr( sprintf( __( 'Personagem: %s', 'geek-ao-cubo' ), $name ) );
...
if ( ! empty( $role ) ) {
	...
	$main_roles = array(
		'principal', 'main', 'protagonista', 'antagonista', 'deuteragonista', 
		'tritagonista', 'anti-herói', 'anti-heroi', 'vilão', 'vilao', 
		'arqui-inimigo', 'arqui-inimiga', 'rival'
	);

	$supporting_roles = array(
		'secundário', 'secundario', 'supporting', 'mentor', 'confidente', 
		'par romântico', 'par romantico', 'sidekick', 'aliado', 'figura parental', 
		'guardião', 'guardiao', 'alívio cômico', 'alivio comico', 'foil',
		'coadjuvante', 'mensageiro', 'camaleão', 'camaleao', 'shapeshifter',
		'personagem coral', 'personagem de apoio', 'apoio', 'escudeiro', 'companheiro',
		'contraponto'
	);

	$is_main = false;
	foreach ( $main_roles as $r ) {
		if ( str_contains( $role_lower, $r ) ) {
			$is_main = true;
			break;
		}
	}
	...
	<div class="card-personagem__poster">
		<?php if ( ! empty( $image_url ) ) : ?>
			<img
				...
				alt="<?php echo esc_attr( sprintf( __( 'Imagem do personagem %s', 'geek-ao-cubo' ), $name ) ); ?>"
				loading="lazy"
				decoding="async"
			/>
		<?php else : ?>
