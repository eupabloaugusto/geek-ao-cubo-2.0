...
if ( $should_use_main_query ) {
	...
	if ( isset( $_GET['category_name'] ) && ! empty( $_GET['category_name'] ) ) {
		$query_args['category_name'] = sanitize_text_field( wp_unslash( $_GET['category_name'] ) );
	}
...
<div class="lista-publicacoes js-ajax-container js-ajax-scroll-target <?php echo $class; ?>">
	...
	<?php else : ?>
		...
		<div class="lista-publicacoes__vazio" style="text-align: center; padding: var(--space-600) var(--space-400); max-width: var(--container-max); margin: 0 auto;">
			<h2><?php esc_html_e( 'Nenhuma publicação encontrada.', 'geek-ao-cubo' ); ?></h2>
			<p><?php esc_html_e( 'Tente alterar os filtros ou realizar uma nova busca.', 'geek-ao-cubo' ); ?></p>
		</div>
