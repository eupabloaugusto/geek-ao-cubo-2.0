		...
		if ( $wp_menu ) {
			...
			foreach ( $menu_items_map as $id => &$item ) {
				if ( $item['parent'] && isset( $menu_items_map[ $item['parent'] ] ) ) {
					$menu_items_map[ $item['parent'] ]['sublinks'][] = &$item;
				}
			}
			unset($item);
...
if ( empty( $menu_items ) ) {
	...
	$menu_items = array(
		array( 'label' => __( 'Início', 'geek-ao-cubo' ),     'url' => home_url( '/' ),           'is_active' => is_front_page() ),
		array( 
			'label'     => __( 'Catálogo', 'geek-ao-cubo' ), 
			...
			'sublinks'  => array(
				array( 'label' => __( 'Anime', 'geek-ao-cubo' ), 'url' => home_url( '/catalogo-de-animes/?tipo_midia=anime' ) ),
				array( 'label' => __( 'Mangá', 'geek-ao-cubo' ), 'url' => home_url( '/catalogo-de-animes/?tipo_midia=manga' ) ),
			),
		),
		array( 'label' => __( 'Temporadas', 'geek-ao-cubo' ), 'url' => home_url( '/temporada/' ), 'is_active' => trailingslashit( home_url( '/temporada/' ) ) === $current_url || is_singular( 'temporada' ) ),
		array( 'label' => __( 'Calendário', 'geek-ao-cubo' ), 'url' => home_url( '/calendario/' ),'is_active' => trailingslashit( home_url( '/calendario/' ) ) === $current_url ),
	);
}
...
<aside id="nav-drawer" class="<?php echo esc_attr( $classes ); ?>" role="dialog" aria-modal="true" aria-hidden="<?php echo $is_open ? 'false' : 'true'; ?>" aria-labelledby="nav-drawer-title">
	<div class="navigation-drawer__panel">
		
		<!-- Cabeçalho do Drawer -->
		<header class="navigation-drawer__header">
			<div class="navigation-drawer__logo" id="nav-drawer-title">
				<?php 
		...
		<div class="navigation-drawer__body">
			...
			<?php if ( $search_enabled ) : ?>
				<div class="navigation-drawer__search">
					<?php mm_render_component( 'atoms', 'input-busca-compact', array(
						'placeholder' => __( 'Pesquisar no site...', 'geek-ao-cubo' ),
						'readonly'    => true,
			...
			<!-- Navegação Principal -->
			<nav class="navigation-drawer__nav" role="navigation" aria-label="<?php esc_attr_e( 'Menu Mobile', 'geek-ao-cubo' ); ?>">
