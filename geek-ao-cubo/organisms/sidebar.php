...
<aside class="sidebar <?php echo $class; ?>" role="complementary" aria-label="<?php esc_attr_e( 'Barra Lateral', 'geek-ao-cubo' ); ?>">
	
	<!-- Seção/Widget: Artigos Recentes (Variação Lista Horizontal) -->
	<section class="sidebar-widget-clean">
		<h3 class="sidebar-widget__title" style="margin-left: var(--space-300);"><?php _e( 'Últimas Novidades', 'geek-ao-cubo' ); ?></h3>
		...
		<div class="sidebar-posts-column" style="display: flex; flex-direction: column; gap: var(--space-400);">
			...
			if ( $query_sidebar->have_posts() ) {
				...
				?>
				<p class="sidebar-posts-empty" style="padding-inline: var(--space-400); color: var(--neutral-400); font-family: var(--font-body); font-size: var(--text-xs-size);"><?php _e( 'Nenhum artigo disponível.', 'geek-ao-cubo' ); ?></p>
				<?php
			}
			?>
		</div>
