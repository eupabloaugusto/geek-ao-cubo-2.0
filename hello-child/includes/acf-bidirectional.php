...
// =========================================================================
// Hook: Sincroniza Episódio ↔ Anime
// Disparado após o ACF salvar os campos de qualquer post do tipo 'episodio'
// =========================================================================

/**
 * Sincroniza ep_anime_relacionado → anime_episodios
 *
 * @param int $post_id ID do post recém-salvo.
 */
function mm_sync_episodio_anime( $post_id ) {
	if ( get_post_type( $post_id ) !== 'episodio' ) {
		return;
	}

	mm_sync_bidirectional_relationship(
		$post_id,
		'ep_anime_relacionado',  // Campo do Episódio que aponta para o Anime
		'anime_episodios',       // Campo do Anime que lista os Episódios (oculto/hidden)
		'anime'
	);
}
add_action( 'acf/save_post', 'mm_sync_episodio_anime', 20 );
