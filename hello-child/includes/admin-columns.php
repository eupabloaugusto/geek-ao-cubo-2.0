	...
	switch ( $column ) {
		case 'anime_capa':
			...
			if ( $thumb ) {
				...
				$capa_url = get_field( 'anime_imagem_capa_url', $post_id );
				if ( $capa_url ) {
		...
		case 'anime_nota':
			$nota = get_field( 'anime_nota_mal', $post_id );
			if ( $nota ) {
...
function mm_anime_overview_metabox_content( $post ) {
	...
	$nota       = get_field( 'anime_nota_mal', $post->ID );
	$ranking    = get_field( 'anime_ranking', $post->ID );
	$membros    = get_field( 'anime_membros', $post->ID );
	$studio     = get_field( 'anime_studio', $post->ID );
