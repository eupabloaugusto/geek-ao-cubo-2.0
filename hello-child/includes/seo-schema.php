...
function mm_inject_seo_json_ld() {
	...
	switch ( $post_type ) {
		...
		case 'anime':
			...
			$studio      = get_field( 'anime_studio', $post_id );
			$ano         = get_field( 'anime_ano', $post_id );
			$sinopse     = get_field( 'anime_sinopse', $post_id );
			$score       = get_field( 'anime_nota_mal', $post_id );
			$membros     = get_field( 'anime_membros', $post_id );
			$poster      = get_the_post_thumbnail_url( $post_id, 'large' );

			if ( empty( $poster ) ) {
				$poster = get_field( 'anime_imagem_capa_url', $post_id );
			}
			...
			if ( ! empty( $anime_rel ) ) {
				...
				if ( $anime_id ) {
					...
					if ( empty( $anime_poster ) ) {
						$anime_poster = get_field( 'anime_imagem_capa_url', $anime_id );
					}
