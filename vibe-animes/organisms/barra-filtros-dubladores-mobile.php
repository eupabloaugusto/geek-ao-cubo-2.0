<?php
/**
 * Organism: Barra de Filtros Mobile Dubladores (barra-filtros-dubladores-mobile)
 *
 * Painel de filtros otimizado para telas touch.
 * Clone estrutural de barra-filtros-personagens-mobile.php — apenas filtros específicos de dublador.
 *
 * @package vibe-animes
 *
 * @param string $class      Classes CSS adicionais para a barra sticky.
 * @param string $action_url URL de destino do formulário. Default: permalink atual.
 */
...
$sheet_id   = 'barra-filtros-dubladores-sheet';
$form_id    = 'form-filtros-dubladores-mobile';
...
$sel_idioma     = isset( $_GET['idioma'] )    ? array( sanitize_text_field( wp_unslash( $_GET['idioma'] ) ) ) : array();

$count_ativos = count( array_filter( $sel_ordem ) ) + count( array_filter( $sel_idioma ) );
...
<div class="barra-filtros-mobile <?php echo $class; ?>">
	<form
		...
		id="<?php echo $form_id; ?>"
		role="search"
		aria-label="<?php esc_attr_e( 'Filtrar dubladores', 'vibe-animes' ); ?>"
	>
		<?php
		...
		<div class="barra-filtros-mobile__bar">
			...
			<label class="barra-filtros-mobile__search-wrap" for="filtros-dubladores-mobile-busca">
				...
				<input
					type="search"
					id="filtros-dubladores-mobile-busca"
					name="busca"
					class="barra-filtros-mobile__search-input"
					placeholder="<?php esc_attr_e( 'Buscar dublador...', 'vibe-animes' ); ?>"
		...
		<?php if ( $count_ativos > 0 ) : ?>
			<div class="barra-filtros-mobile__ativos" aria-label="<?php esc_attr_e( 'Filtros ativos', 'vibe-animes' ); ?>">
				...
				<?php endforeach; ?>
				<?php foreach ( $sel_idioma as $slug ) :
					if ( empty( $slug ) ) continue;
					$label = isset( $idioma_options[ $slug ] ) ? $idioma_options[ $slug ] : $slug;
				?>
					<span class="barra-filtros-mobile__ativo-chip barra-filtros-mobile__ativo-chip--idioma">
						<?php echo esc_html( $label ); ?>
						<button type="button" class="barra-filtros-mobile__ativo-remove" data-filtros-remove="idioma" data-filtros-value="<?php echo esc_attr( $slug ); ?>" aria-label="<?php echo esc_attr( sprintf( __( 'Remover filtro %s', 'vibe-animes' ), $label ) ); ?>">×</button>
					</span>
				<?php endforeach; ?>
			</div>
		<?php endif; ?>
