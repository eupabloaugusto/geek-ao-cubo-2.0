...
$sel_categoria = isset( $_GET['categoria_slug'] ) ? sanitize_text_field( wp_unslash( $_GET['categoria_slug'] ) ) : '';
...
if ( ! empty( $sel_categoria ) ) {
	...
	if ( $cat_obj ) {
		$ativos_html .= sprintf(
			'<span class="barra-filtros-mobile__ativo-chip">%s <button type="button" class="barra-filtros-mobile__ativo-remove" data-remove-select="categoria_slug" aria-label="%s">&times;</button></span>',
			esc_html( $cat_obj->name ),
			esc_attr( sprintf( __( 'Remover filtro %s', 'vibe-animes' ), $cat_obj->name ) )
		);
	}
}
...
if ( ! empty( $sel_ordem ) && 'recentes' !== $sel_ordem && isset( $ordem_options[ $sel_ordem ] ) ) {
	$ativos_html .= sprintf(
		...
		esc_attr( sprintf( __( 'Remover filtro %s', 'vibe-animes' ), $ordem_options[ $sel_ordem ] ) )
	);
}
?>
...
<div class="barra-filtros-mobile <?php echo $class; ?>">
	...
	<form method="get" action="<?php echo $action; ?>" class="barra-filtros-mobile__form" id="form-filtros-pub-mobile" role="search">
		...
		<div class="barra-filtros-mobile__bar">
			<label class="barra-filtros-mobile__search-wrap" for="filtros-pub-mobile-busca">
				...
				<input 
					...
					placeholder="<?php esc_attr_e( 'Buscar...', 'vibe-animes' ); ?>" 
		...
		<div id="mobile-filters-panel" class="barra-filtros-mobile__sheet" role="dialog" aria-modal="true" aria-label="<?php esc_attr_e( 'Filtros', 'vibe-animes' ); ?>" aria-hidden="true">
			
			<!-- Handle de arrastar -->
			<div class="barra-filtros-mobile__handle" aria-hidden="true"></div>

			<div class="barra-filtros-mobile__sheet-header">
				<h3 class="barra-filtros-mobile__sheet-title"><?php esc_html_e( 'Filtros', 'vibe-animes' ); ?></h3>
				<button type="button" class="barra-filtros-mobile__close barra-filtros-mobile__panel-close" aria-label="<?php esc_attr_e( 'Fechar filtros', 'vibe-animes' ); ?>">
			...
			<fieldset class="barra-filtros-mobile__fieldset">
				<legend class="barra-filtros-mobile__legend"><?php esc_html_e( 'Categorias', 'vibe-animes' ); ?></legend>
			...
			<fieldset class="barra-filtros-mobile__fieldset">
				<legend class="barra-filtros-mobile__legend"><?php esc_html_e( 'Ordenar por', 'vibe-animes' ); ?></legend>
				<?php mm_render_component( 'molecules', 'form-field', array(
					'name'        => 'ordem',
					'type'        => 'select',
