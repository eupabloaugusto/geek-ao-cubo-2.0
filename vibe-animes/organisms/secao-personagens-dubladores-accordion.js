/**
 * Organism JS: Seção de Personagens e Dubladores em Acordeão (secao-personagens-dubladores-accordion)
 *
...
( function () {
	...
	function initAccordions() {
		const triggers = document.querySelectorAll( '.secao-personagens-dubladores-accordion .js-accordion-trigger' );
		...
		triggers.forEach( trigger => {
			trigger.addEventListener( 'click', function () {
				...
				if ( isOpen ) {
					...
					// Recalcula a paginação ao abrir (caso tenha mudado de tamanho de tela enquanto fechado)
					const moreBtn = item.querySelector( '.js-dub-more' );
					if ( moreBtn ) {
	...
	function getActiveColumns( gridElement ) {
		...
		// Conta a quantidade de valores separados por espaço (ex: "320px 320px" = 2 colunas)
	...
	function updatePaginationState( item ) {
		const grid      = item.querySelector( '.js-dub-grid' );
		const cards     = item.querySelectorAll( '.js-dub-card' );
		const actions   = item.querySelector( '.js-dub-actions' );
		const moreBtn   = item.querySelector( '.js-dub-more' );
		const lessBtn   = item.querySelector( '.js-dub-less' );
		...
		// Se está na inicialização (current === 0), configura a visualização inicial
		if ( current === 0 ) {
			if ( total > limit ) {
			...
			// Se a tela mudou de tamanho, ajustamos dinamicamente as visibilidades baseadas no limite recalculado
	...
	function initEpisodePagination() {
		const items = document.querySelectorAll( '.secao-personagens-dubladores-accordion .js-accordion-item' );
		...
		items.forEach( item => {
			...
			moreBtn.addEventListener( 'click', function () {
				...
				// Revela as duplas do lote seguinte
				cards.forEach( card => {
					const idx = parseInt( card.dataset.index, 10 );
