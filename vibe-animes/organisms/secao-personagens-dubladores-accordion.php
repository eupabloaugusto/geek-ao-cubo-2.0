...
$titulo     = isset( $args['titulo'] ) ? esc_html( $args['titulo'] ) : __( 'Dubladores', 'vibe-animes' );
...
if ( empty( $grupos ) && ! empty( $dubladores ) ) {
	foreach ( $dubladores as $dublador ) {
		...
		if ( strtolower( $lang ) === 'japanese' || strtolower( $lang ) === 'japonês' || strtolower( $lang ) === 'japones' ) {
			$grupo_nome = __( 'Dublagem Japonesa (Original)', 'vibe-animes' );
		} elseif ( strtolower( $lang ) === 'portuguese' || strtolower( $lang ) === 'português' || strtolower( $lang ) === 'portugues' || strtolower( $lang ) === 'brazilian' ) {
			$grupo_nome = __( 'Dublagem Brasileira (Nacional)', 'vibe-animes' );
		} else {
			$grupo_nome = sprintf( __( 'Dublagem %s', 'vibe-animes' ), $lang );
		}
				...
				<div class="secao-personagens-dubladores-accordion__item js-accordion-item" <?php echo $is_open ? 'data-state="open"' : ''; ?>>
					...
					>
						...
						<span class="secao-personagens-dubladores-accordion__trigger-badge">
							<?php echo sprintf( _n( '%s integrante', '%s integrantes', $total_cards, 'vibe-animes' ), $total_cards ); ?>
						</span>
					...
					>
						<div class="secao-personagens-dubladores-accordion__content-inner">
							...
							<div class="secao-personagens-dubladores-accordion__grid js-dub-grid">
								<?php foreach ( $dubladores as $card_idx => $dublador ) : 
									...
									if ( empty( $dub_array['character_name'] ) ) {
										$dub_array['character_name'] = __( 'Personagem', 'vibe-animes' );
									}
								?>
							...
							<div class="secao-personagens-dubladores-accordion__actions js-dub-actions" style="display: none;">
								...
								<button 
									type="button" 
									class="btn btn--secondary btn-dub-action--less js-dub-less"
									style="display: none;"
								>
									...
									<span><?php _e( 'Ver menos', 'vibe-animes' ); ?></span>
								</button>

								<button 
									type="button" 
									class="btn btn--primary btn-dub-action--more js-dub-more"
									data-total="<?php echo $total_cards; ?>"
									data-current="0"
								>
									<span><?php _e( 'Ver mais', 'vibe-animes' ); ?></span>
									<svg style="width: 1.25rem; height: 1.25rem;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
										<polyline points="6 9 12 15 18 9"></polyline>
									</svg>
								</button>
