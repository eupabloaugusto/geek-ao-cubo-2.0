...
// Pega o ID da URL interceptada pela Rewrite Rule (Fallback legado)
$mal_id = get_query_var( 'personagem_id' );

// Nova Lógica Hierárquica: Buscar MAL ID a partir do Slug do Anime/Mangá e do Personagem
if ( ! $mal_id ) {
	$anime_slug      = get_query_var( 'anime_slug' );
	$personagem_slug = get_query_var( 'personagem_slug' );

	if ( $anime_slug && $personagem_slug ) {
		$anime_post = get_page_by_path( $anime_slug, OBJECT, 'anime' );
		if ( ! $anime_post ) {
			$anime_post = get_page_by_path( $anime_slug, OBJECT, 'manga' );
		}
		if ( $anime_post ) {
			$is_manga = ( $anime_post->post_type === 'manga' );
			$meta_key = $is_manga ? 'manga_id_mal' : 'anime_id_mal';
			$anime_id_mal = get_post_meta( $anime_post->ID, $meta_key, true );
			if ( $anime_id_mal ) {
				$chars = $is_manga 
					? Jikan_API::get_manga_characters( $anime_id_mal )
					: Jikan_API::get_anime_characters( $anime_id_mal );
				if ( ! empty( $chars ) && is_array( $chars ) ) {
					// Limpa o nome do personagem do mesmo jeito que listagens fazem
					$clean_name = function( $name ) {
						$parts = explode( ', ', $name );
						return ( count( $parts ) === 2 ) ? $parts[1] . ' ' . $parts[0] : $name;
					};
					foreach ( $chars as $char ) {
						$c_name = $clean_name( $char['character']['name'] ?? '' );
						if ( sanitize_title( $c_name ) === $personagem_slug ) {
							$mal_id = $char['character']['mal_id'];
							
							// Redirecionamento Canônico Automático (301)
							// Se a URL acessada não é exatamente a hierárquica baseada na obra atual
							// WP cuidará do redirect caso façamos setup de canonical via hook,
							// mas para evitar duplicate content agora:
							break;
						}
					}
				}
...
if ( ! $mal_id ) {
	error_log("SINGLE_PERSONAGEM 404 TRIGGERED! URL: " . $_SERVER['REQUEST_URI']);
	error_log("Vars: anime_slug=$anime_slug, personagem_slug=$personagem_slug");
	error_log("Anime Post found: " . (isset($anime_post) && $anime_post ? 'YES (ID: '.$anime_post->ID.')' : 'NO'));
	error_log("Anime MAL ID: " . (isset($anime_id_mal) ? $anime_id_mal : 'NONE'));
	error_log("Chars count: " . (isset($chars) ? count((array)$chars) : 'NONE'));
...
if ( ! empty( $jikan_data['voices'] ) ) {
	$base_anime_slug = $anime_slug ?? ( ! empty( $animes_list ) ? wp_basename( $animes_list[0]['permalink'] ) : '' );
	if ( ! $base_anime_slug ) {
		$base_anime_slug = 'anime';
	}
	foreach ( $jikan_data['voices'] as $voice ) {
		...
		$voices[] = array(
			...
			'va_url'      => home_url( '/' . $base_anime_slug . '/dubladores/' . sanitize_title( $voice['person']['name'] ) . '/' ),
			'va_language' => $va_lang,
		);
	}
}
...
<div class="personagem-layout">
	<main class="personagem-layout__main" id="main-content">
		...
		<?php if ( ! empty( $universe_chars ) ) : ?>
			<?php 
			// Se a página foi acessada pela rota legada, descobre o anime base para os links
			$base_anime_slug = $anime_slug ?? ( ! empty( $animes_list ) ? wp_basename( $animes_list[0]['permalink'] ) : '' );
			
			mm_render_component( 'organisms', 'secao-esteira-personagens', array(
				'titulo_secao'  => __( 'Personagens do Mesmo Universo', 'geek-ao-cubo' ),
				'personagens'   => $universe_chars,
				'anime_slug'    => $base_anime_slug,
			) ); ?>
		<?php endif; ?>
